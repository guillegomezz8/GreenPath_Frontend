...
// Layout reutilizable para listados con header, filtros, contadores y paginacion.
// Si layout="list", no se aplica grid por defecto.
  ...
  const defaultCountCard = (def, value) => (
    <Card key={def.key} className="overflow-hidden">
      <CardContent className="pt-4 pb-4 px-4 text-center">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mt-1 leading-tight">{def.label}</p>
      </CardContent>
    </Card>
  );
        ...
        <>
            ...
            <Card>
              <CardContent className="p-0">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-5 h-5 text-muted-foreground transition-transform ${isCountsOpen ? "rotate-180" : ""}`}
                  />
                </button>
                ...
                {isCountsOpen && (
                  <div className="border-t p-4 space-y-3">
                    {countDefs.map((def) => (
                      <div key={def.key} className="flex items-center justify-between py-2 border-b last:border-b-0">
          ...
          <div
            className={`hidden md:grid ${
              countsWrapperClassName || `grid-cols-1 ${responsiveColsClass} gap-3 md:gap-4`
            }`}
          >
            {countDefs.map((def) =>
              renderCountCard ? renderCountCard(def, counts[def.key]) : defaultCountCard(def, counts[def.key])
            )}
          </div>
        </>
      )}

      <div className={layout === "list" ? "w-full space-y-4" : contentClassName}>
        {loading
          ? loadingNode || (
              <Card className="col-span-full">
                <CardContent className="text-center py-8 md:py-12">
                  <div className="animate-pulse">
                    <div className="text-sm md:text-base text-muted-foreground">Cargando...</div>
                  </div>
                </CardContent>
              </Card>
            )
            ...
            <div className={`flex flex-col sm:flex-row items-center justify-between gap-3 ${paginationClassName}`}>
              <span className="text-xs sm:text-sm text-muted-foreground order-2 sm:order-1">
                <span className="hidden sm:inline">
                  {total.toLocaleString()} resultado{total === 1 ? "" : "s"} · Pagina {page} de {totalPages}
                </span>
              ...
              <div className="flex items-center gap-2 order-1 sm:order-2">
                <Button variant="outline" size="sm" disabled={page <= 1} onClick={onPrevPage} className="text-xs sm:text-sm">
                  <span className="hidden sm:inline">Anterior</span>
                  <span className="sm:hidden">Ant.</span>
                </Button>
                <span className="text-xs sm:text-sm text-muted-foreground px-2">{page}</span>
                <Button
                  variant="outline"
                  size="sm"
          ...
          <Card>
            <CardContent className="text-center py-8 md:py-12">
              <div className="max-w-md mx-auto space-y-3">
                ...
                <p className="text-sm md:text-base text-muted-foreground leading-relaxed">{emptyText}</p>
              </div>
            </CardContent>
          </Card>
        )
